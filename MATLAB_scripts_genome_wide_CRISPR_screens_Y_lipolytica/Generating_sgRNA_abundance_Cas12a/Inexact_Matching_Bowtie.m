...
% Inputs: 
% 1. Examplefile1_Cas12a.bam. Obtained as a result of running bowtie on
% 'Examplefile1_Cas12a.fastqsanger' against the genome file
% 'CLIB89+control_Cas12a.fasta'.
% 2. All_sgRNA_pos_Cas12a.mat: variable file containg genomic start positions of 
...
tic
clear
bamFilename = 'Examplefile1_Cas12a.bam'; %This file will be the result of running bowtie on 'Examplefile1_Cas12a.fastqsanger' against the genome file 'CLIB89+control_Cas12a.fasta'
...
Aflag(Aflag==16)=1; %The reverse strand alignment is indicated as the number 16 on Bowtie, that is chnaged to 1 here 
Bflag(Bflag==16)=1;
Cflag(Cflag==16)=1;
Dflag(Dflag==16)=1;
Eflag(Eflag==16)=1;
...
Ap=cell2mat(All_sgRNA_pos_Cas12a(:,3));
c=1;
for i=1:length(Ap)-1
    diff=Ap(i+1)-Ap(i);
    ones(c)=diff;
